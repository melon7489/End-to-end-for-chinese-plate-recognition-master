...
def locate_and_correct(img_src, img_mask):
    ...
    img_mask = cv2.cvtColor(thresh, cv2.COLOR_GRAY2RGB)
    ...
    if not len(contours):  # contours1长度为0说明未检测到车牌
        ...
        for ii, cont in enumerate(contours):
            x, y, w, h = cv2.boundingRect(cont)  # 获取外接矩形左上角坐标以及宽和高
            img_cut_mask = img_mask[y:y + h, x:x + w]  # 将标签车牌区域截取出来,注意图片的坐标和矩阵坐标转换
            #
            ...
            if np.mean(img_cut_mask) >= 75 and w > 15 and h > 15:
                ...
                cv2.drawContours(img_mask, contours, -1, (0, 0, 255), 2)  # 轮廓，按列表顺序连接画线
                cv2.drawContours(img_mask, [box], 0, (0, 255, 0), 2)  # 最小外接矩形
                # cv2.imshow('img_mask',img_mask)
                # cv2.waitKey(0)
                ...
                def point_to_line_distance(X, Y):
                    if x2 - x0:
                        ...
                        d_up = abs(k_up * X - Y + y2 - k_up * x2) / (k_up ** 2 + 1) ** 0.5  # # 点到直线的距离公式
                    else:  # 斜率无穷大
                        d_up = abs(X - x2)
                    if x1 - x3:
                ...
                lic = cv2.warpPerspective(img_src, transform_mat, (240, 80))  # 透视变换进行车牌矫正
                ...
                Lic_img.append(lic)
                cv2.drawContours(img_src_copy, [np.array([l0, l1, l3, l2])], -1, (0, 0, 255), 2)  # 注意轮廓的连接顺序，在img_src_copy上绘制出精准定位的车牌轮廓，(0, 255, 0)表示绘制线条为绿色
