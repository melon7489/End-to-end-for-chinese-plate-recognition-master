...
def cnn_train():
    ...
    # 读取数据集
    path = './cnndatasets/'  # 车牌号数据集路径(车牌图片宽240，高80)
    pic_name = sorted(os.listdir(path))
    n = len(pic_name)
    X_train, y_train = [], []
    for i in range(n):
        print("正在读取第%d张图片" % i)
        img = cv2.imdecode(np.fromfile(path + pic_name[i], dtype=np.uint8), -1)  # cv2.imread无法读取中文路径图片，改用此方式
    ...
    plot_model(model, to_file="./CNN.png", show_shapes=True, rankdir='TB')
    ...
    checkpoint = ModelCheckpoint('cccccnn.h5', save_best_only=True)
    # 模型训练
    print("开始训练cnn")
    log = model.fit(X_train, y_train, epochs=35, batch_size=10, callbacks=[checkpoint], validation_split=0.1)  # 总loss为7个loss的S
    model.save('cnn.h5')
    print('cnn.h5保存成功!!!')
